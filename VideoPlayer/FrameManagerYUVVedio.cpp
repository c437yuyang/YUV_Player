...
void rgb2Mat(byte *pSrc, Mat &dst, int width, int height);
CFrameManagerYUVVedio::CFrameManagerYUVVedio()
{
}
...
{
	m_nFrameSize = width*height*1.5;
	m_nFrameHeight = height;
	m_nFrameWidth = width;

	USES_CONVERSION;
	string fileName = W2A(YUVFile.GetBuffer());
	m_ifs.open(fileName, ios::binary); //ifstream竟然也可以直接打开CString类型的参数。。
	if (!m_ifs.is_open())
	{
		::AfxMessageBox(_T("打开输入文件失败!"));
		return;
	}

	long fileSize = CCommon::GetFileSize(fileName);

	/*CFile file;
	file.Open(YUVFile, CFile::modeRead);
	printf("%ls", file.GetFileName());
	...
	cout << "共:" << m_frmIdXCtl.GetFrameCount() << "帧" << endl;
#endif // VERBOSE
...
void rgb2Mat(byte *pSrc, Mat &dst, int width, int height)
{
	...
	for (int i = 0; i != height; ++i)
	{
		...
		{
			dst.at<Vec3b>(i, j)[0] = pSrc[i*width * 3 + j * 3];
			dst.at<Vec3b>(i, j)[1] = pSrc[i*width * 3 + j * 3 + 1];
			dst.at<Vec3b>(i, j)[2] = pSrc[i*width * 3 + j * 3 + 2];
		}
	}
}